...
wrong tags. This catches the rest:
  ...
  - cross-page links to a heading that does not exist
...
def main():
    ...
    for p in pages:
        ...
        for target, frag in ANCHOR.findall(text):
            checked_anchors += 1
            page_path = (p.parent / target).resolve()
            heads = headings_of(page_path)
            if heads is None or frag not in heads:
                bad_anchors.append((p.relative_to(ROOT), target + "#" + frag))
    ...
    print("pages: %d   internal links: %d   anchors: %d   citations checked: %d"
          % (len(pages), len(linked), checked_anchors, checked_cites))
    if not SRC.exists():
    ...
    if bad_anchors:
        fail = True
        print("\n%d link(s) to a heading that does not exist:" % len(bad_anchors))
        for page, target in bad_anchors:
            print("  %s" % page)
            print("      %s" % target)
