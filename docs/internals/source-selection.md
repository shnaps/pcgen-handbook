...
`SourceMigration` runs when a **saved character** is opened, not when data loads. It maps a campaign key recorded in a `.pcg` file to its current name. The rules come
